...
class SAGNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.em4 = olm(2)
        self.model = SAGNN_model(classes=2, zoom_factor=8, \
                  criterion=nn.CrossEntropyLoss(ignore_index=255), \
                  pretrained=True, shot=1,
                  no_pass=1)
    ...
    def forward(self, query_img, query_img_th, support_img, support_img_th, support_mask):
        logit_mask = self.model(query_img, support_img, support_mask)
        logit_mask_th = self.model(query_img_th, support_img_th, support_mask)
        logit_mask_s = self.em4(logit_mask_th, logit_mask)

        return logit_mask_s

class SAGNN_model(nn.Module):

    def __init__(self, layers=50, classes=2, zoom_factor=8,
                 criterion=nn.CrossEntropyLoss(ignore_index=255),
                 pretrained=True, shot=1, ppm_scales=[60, 15, 8], no_pass=1, device_number=0):
        super(SAGNN_model, self).__init__()
        ...
        # self.layer0, self.layer1, self.layer2, self.layer3, self.layer4 = get_vgg16_layer(backbone)
    ...
    def forward(self, x, s_x=torch.FloatTensor(1, 1, 3, 473, 473).cuda(0), s_y=torch.FloatTensor(1, 1, 473, 473).cuda(0)):
        # print(x.size())
        # print(s_x.size())
        # print(s_y.size())
        s_x = s_x.unsqueeze(1)
...
if __name__ == "__main__":
    ...
    net.to(device)
    query_set = torch.rand(size=(4, 3, 473, 473), device="cuda:0")
    support_set = torch.rand(size=(4, 1, 3, 473, 473), device="cuda:0")
    support_masks = torch.randint(low=0, high=1, size=(4, 1, 473, 473), device="cuda:0")
    a = net(query_set, support_set, support_masks)