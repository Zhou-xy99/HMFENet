...
from model.base.decoder import build_decoder
from model.base.ASPP1 import ASPP
from model.base.seed_init import place_seed_points
...
class ASGN(nn.Module):

    def __init__(self, args):
        super(ASGN, self).__init__()
        self.args = args
        self.em4 = olm(2)
        self.model = ASGN_model(args=self.args)
        self.criterion = nn.CrossEntropyLoss(ignore_index=255)

    def forward(self, query_img, query_img_th, support_img, support_img_th, support_mask, y):
        logit_mask, aux_loss = self.model(query_img, support_img, support_mask, y)
        logit_mask_th, aux_loss_th = self.model(query_img_th, support_img_th, support_mask, y)
        logit_mask_s = self.em4(logit_mask_th.float(), logit_mask.float())

        main_loss = self.criterion(logit_mask_s, y.long())
        aux_loss_s = 0.5*aux_loss + 0.5*aux_loss_th
        return logit_mask_s, main_loss, aux_loss_s

class ASGN_model(nn.Module):
    def __init__(self, args):
        super(ASGN_model, self).__init__()
        ...
        self.eval_iter = 5
        self.pyramid = True
        self.ppm_scales = [60, 15, 8]
        self.bsz = args.bsz

        resnet.BatchNorm = BatchNorm
    ...
    def forward(self, x, s_x=torch.FloatTensor(1,1,3,473,473).cuda(), s_y=torch.FloatTensor(1,1,473,473).cuda(), y=None):
        ...
        if self.training:
            s_seeds = []
            for j in range(0, self.bsz):
                init_seed_list = []
                for i in range(0, self.shot):
                    mask = (s_y[j][i, :, :] == 1).float()  # H x W
                    init_seed = place_seed_points(mask, down_stride=8, max_num_sp=5, avg_sp_area=100)
                    init_seed_list.append(init_seed.unsqueeze(0))
                init_seed_lists = torch.cat(init_seed_list, 0)
                s_seeds.append(init_seed_lists.unsqueeze(0))  # (shot, max_num_sp, 2)
            s_seed = torch.cat(s_seeds, 0).cuda()  # (shot, max_num_sp, 2)
            ...
            for i in range(0, self.shot):
                ...
                init_seed = place_seed_points(mask, down_stride=8, max_num_sp=5, avg_sp_area=100)
                init_seed_list.append(init_seed.unsqueeze(0))
            s_seed = torch.cat(init_seed_list, 0).unsqueeze(0).cuda()
...
########################### Context Module ###########################
        ...
        return out.max(1)[1].unsqueeze(1), aux_loss
