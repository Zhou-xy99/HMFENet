...
class Correlation:
    ...
    def multilayer_correlation(cls, query_feats, support_feats, stack_ids):
        ...
        for idx, (query_feat, support_feat) in enumerate(zip(query_feats, support_feats)):
            ...
            x = torch.bmm(query_feat.transpose(1, 2), support_feat)#torch.Size([1, 50, 50, 50, 50])
            x_mean = torch.mean(x, dim=1, keepdim=True)
            x_var = torch.var(x, dim=1, keepdim=True)
            x = torch.div(x - x_mean, torch.sqrt(x_var + eps))
            x1 = F.softmax(x, dim=1)
            x_mean1 = torch.mean(x1, dim=2, keepdim=True)
            x_var1 = torch.var(x1, dim=2, keepdim=True)
            x1 = torch.div(x1 - x_mean1, torch.sqrt(x_var1 + eps))
            corr = F.softmax(x1, dim=2)

            corr = corr.view(bsz, ha, wa, hb, wb)
