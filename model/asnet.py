...
from model.base.feature1 import extract_feat_vgg, extract_feat_res
from model.base.correlation3 import Correlation
from common import utils
from common.evaluation import Evaluator

from model.learner1 import AttentionLearner
from common.olm4 import olm
# HPNLearner AttentionLearner
...
class AttentiveSqueezeNetwork(pl.LightningModule):
    ...
    def forward(self, query_img, query_img_th, support_img, support_img_th, support_mask):
        ...
        logit_mask_th = self.learner(corr_th, support_mask)

        if not self.use_original_imgsize:
            logit_mask = F.interpolate(logit_mask, support_img.size()[2:], mode='bilinear', align_corners=True)
            logit_mask_th = F.interpolate(logit_mask_th, support_img_th.size()[2:], mode='bilinear', align_corners=True)
        logit_mask_fus = self.em4(logit_mask_th, logit_mask)

        return logit_mask_fus
