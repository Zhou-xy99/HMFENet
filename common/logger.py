...
class AverageMeter:
    ...
    def __init__(self, class_ids_interest):
        ...
        self.class_ids_interest = torch.tensor(class_ids_interest).cuda()
    ...
    def update(self, inter_b, union_b, class_id, loss):
        self.intersection_buf.index_add_(1, class_id, inter_b.float())#能实现指定行或列的内容相加的功能
    ...
    def compute_iou(self):
        ...
        iou = iou.index_select(1, self.class_ids_interest)## IoU = TP / (TP + FP + FN)
        miou = iou[1].mean() * 100
    ...
    def write_result(self, split, epoch):
        ...
        msg = '\n*** %s ' % split
        msg += '[@Epoch %02d] ' % epoch
        msg += 'Avg L: %6.5f  ' % loss_buf.mean() #average loss
        msg += 'mIoU: %5.2f   ' % iou
    ...
    def write_process(self, batch_idx, datalen, epoch, write_batch_idx=20):
        if batch_idx % write_batch_idx == 0:
            ...
            iou, fb_iou = self.compute_iou()
            if epoch != -1:
                loss_buf = torch.stack(self.loss_buf)#默认dim=0
                msg += 'L: %6.5f  ' % loss_buf[-1]#每个bz都会新加一个loss
...
class Logger:
    ...
    def initialize(cls, args, training):
        ...
        logpath = args.logpath if training else '_TEST_' + args.load.split('/')[-2].split('.')[0] + logtime  # 文件夹的命名
    ...
    def info(cls, msg):
        ...
        logging.info(msg)#输出日志到屏幕

    @classmethod
    def save_model_miou(cls, model, epoch, val_miou):
        torch.save(model, os.path.join(cls.logpath, 'best_model.pth'))#cls就是指Logger本身，因为Logger初始化的时候已经给了logpath,且方法本身就在Logger类中
    ...
    def log_params(cls, model):
        ...
        Logger.info('Backbone # param.: %d' % backbone_param)#输出
